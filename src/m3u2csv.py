...
# -*- coding: utf-8 -*-
"""
Created on Sat Jan 30 10:01:00 2016
...
try:
    ...
    df = dict(artist=[],title=[],genre=[],length=[],year=[],album=[],origName=[],author=[],identification=[],number=[])
    ...
    for pl in allPlaylists:
        ...
        #playlist[0]=playlist[0][re.search('([CE]:)',playlist[0]).span(1)[0]:len(playlist[0])]
        dfsingle = parse_path_list(playlist)
        #dfsingle = add_tandas.add_tandas(dfsingle)
        dfsingle["author"] = ["behinger"]*len(dfsingle["artist"])
        ...
        #df = dict( (k, [dfsingle[k], df.get(k)]) for k in dfsingle)
        #df = concat([df,dfsingle],ignore_index=True)
    ...
    with os.fdopen(os.open(time.strftime("%Y-%m-%d_%M-%S")+'_output.csv',os.O_CREAT | os.O_RDWR ),'w') as myfile:
    #with open('./csv/output.csv','w') as myfile:
        curr_writer = writer(myfile,lineterminator='\n')
        for r in range(len(df["artist"])):
            curr_writer.writerow([df[out][r].encode('utf-8') if isinstance(df[out][r],str) else df[out][r] for out in df.keys()])
